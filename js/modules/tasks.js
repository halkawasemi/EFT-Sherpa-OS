        ...
        list.innerHTML = filtered.slice(0, search ? 150 : 80).map(t => {
            ...
            const renderObjectives = () => {
                ...
                return t.objectives.map((o, idx) => {
                    ...
                    return `
                    <li class="flex items-start gap-3 p-2 rounded hover:bg-white/5 transition-colors group">
                        <input type="checkbox" 
                               class="mt-1 form-checkbox h-4 w-4 text-tarkov-accent rounded border-gray-600 bg-gray-700 focus:ring-tarkov-accent" 
                               onclick="app.toggleObjectiveProgress('${t.id}', '${o.id || idx}')" 
                               ${isCompleted ? 'checked' : ''}>
                        <div class="flex-1 min-w-0">
                            <div class="text-sm ${isCompleted ? 'text-gray-500 line-through' : 'text-gray-300'}">${o.description}</div>
                            ${requiredCount > 1 || (o.type === 'item' || o.type === 'playerKills' || o.type === 'bossKills') ? `
                            <div class="flex items-center gap-2 mt-1">
                                <div class="flex items-center bg-black/40 rounded border border-gray-700 px-1">
                                    <input type="number" 
