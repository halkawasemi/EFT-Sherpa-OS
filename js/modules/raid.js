...
Object.assign(window.app, {
    ...
    renderMapObjectives(map) {
        const allMapNames = this.data.maps.map(m => m.name);
        
        const tasks = this.data.tasks.filter(t => {
            // 0. Basic Availability Check
            if (t.completed || !this.isTaskAvailable(t)) return false;

            const objectivesText = t.objectives.map(o => o.description).join(' ');

            // 1. Arena Exclusion
            if (objectivesText.includes('in Arena')) return false;

            // Specific Map Assignment Check
            if (t.map && t.map.name !== 'Any') {
                return t.map.name === map.name;
            }

            // 'Any' Map or No Map Assignment Handling
            
            // 2. Excluding Check: e.g. "(excluding Factory)"
            if (objectivesText.includes(`(excluding ${map.name})`)) return false;

            // 3. Contextual Map Check
            // Does it explicitly mention the current map?
            const mentionsCurrentMap = objectivesText.includes(map.name);
            // Does it mention any OTHER map?
            const mentionsOtherMap = allMapNames.some(mName => 
                mName !== map.name && objectivesText.includes(mName) && !objectivesText.includes(`(excluding ${mName})`)
            );

            // If it mentions the current map, show it (even if it mentions others, though unlikely for Any)
            if (mentionsCurrentMap) return true;
            
            // If it mentions other maps but NOT the current one, hide it (it belongs to those other maps)
            if (mentionsOtherMap) return false;

            // If no map names are mentioned, it's a truly generic 'Any' task
            return true;
        });
                    ...
                    <div class="bg-black/30 border border-gray-800 p-3 rounded hover:border-gray-700 transition-colors cursor-pointer" onclick="app.openTask('${t.id}')">
                        <div class="flex justify-between items-start mb-1">
                            ...
                            <div class="flex gap-1">
                                ${(!t.map || t.map.name === 'Any') ? '<span class="text-[8px] px-1 bg-blue-900/30 text-blue-400 rounded border border-blue-800/30 uppercase">Any Map</span>' : ''}
